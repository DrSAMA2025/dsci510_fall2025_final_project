...
# Import path constants from project configuration
from config import DATA_DIR, RESULTS_DIR, GOOGLE_TRENDS_FILE, GOOGLE_TRENDS_ANALYSIS_SUBDIR
...
# --- Define File Paths ---
INPUT_FILE_PATH = DATA_DIR / GOOGLE_TRENDS_FILE
OUTPUT_DIR = RESULTS_DIR / GOOGLE_TRENDS_ANALYSIS_SUBDIR
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)  # Ensure the output directory exists
PLOT_MAIN_PATH = OUTPUT_DIR / 'google_trends_main_analysis.png'
PLOT_CORR_PATH = OUTPUT_DIR / 'google_trends_correlation_heatmap.png'
CSV_SUMMARY_PATH = OUTPUT_DIR / 'google_trends_statistical_summary.csv'

# Read and prepare the data
try:
    df = pd.read_csv(INPUT_FILE_PATH)
except FileNotFoundError:
    print(f"Error: Input file not found at {INPUT_FILE_PATH}. Please run the data loading script first.")
    exit()
...
# Note: Keeping these event definitions here is fine as they are specific to this analysis.
FDA_EVENTS = {
...
# Ensure the date slicing works based on your data structure
august_2025 = df.loc['2025-08-01':'2025-09-30']
...
plt.tight_layout()
plt.savefig(PLOT_MAIN_PATH, dpi=300, bbox_inches='tight')
...
plt.tight_layout()
plt.savefig(PLOT_CORR_PATH, dpi=300, bbox_inches='tight')
...
for event_name, event_date in FDA_EVENTS.items():
    ...
    pre_event = df.loc[event_date - pd.Timedelta(weeks=4):event_date - pd.Timedelta(days=1)]
    post_event = df.loc[event_date:event_date + pd.Timedelta(weeks=4)]

    if len(pre_event) > 0 and len(post_event) > 0:
        event_results[event_name] = {}
        for keyword in ['MASLD', 'NAFLD', 'Wegovy', 'Ozempic']:
            # Check if the keyword column exists to avoid errors if keywords change
            if keyword in df.columns:
                ...
                print(f"Warning: Keyword '{keyword}' not found in data for event study.")

# 4. STATISTICAL SUMMARY
print("Generating statistical summary...")
# Ensure columns exist before describing
summary_cols = [col for col in ['MASLD', 'NAFLD', 'Rezdiffra', 'Wegovy', 'Ozempic'] if col in df.columns]
...
# Check for MASLD column existence before continuing summary prints
if 'MASLD' in df.columns and 'NAFLD' in df.columns:
    print(f"\nTerminology Transition:")
    # Ensure MASLD appears before trying to find the first index
    if (df['MASLD'] > 0).any():
        masld_first_appearance = df[df['MASLD'] > 0].index[0]
        print(f"• MASLD first appeared: {masld_first_appearance.strftime('%Y-%m-%d')}")
    else:
        print("• MASLD has not appeared in the search interest data.")

    print(f"• Current MASLD/NAFLD ratio: {df['MASLD_NAFLD_Ratio'].iloc[-1]:.2f}")

if 'Wegovy' in df.columns and 'Ozempic' in df.columns:
...
if 'MASLD' in df.columns and 'NAFLD' in df.columns:
    ...
    # Use .loc for precise date slicing
    august_spike = df.loc['2025-08-01':'2025-08-31']

    if not august_spike.empty:
        max_masld = august_spike['MASLD'].max()
        max_nafld = august_spike['NAFLD'].max()
        # Ensure mean is not 0 before calculating multiplier
        masld_mean = df['MASLD'].mean()
        nafld_mean = df['NAFLD'].mean()

        masld_multiplier = f"{max_masld / masld_mean:.1f}x average" if masld_mean > 0 else "N/A"
        nafld_multiplier = f"{max_nafld / nafld_mean:.1f}x average" if nafld_mean > 0 else "N/A"

        print(f"• MASLD peak: {max_masld} ({masld_multiplier})")
        print(f"• NAFLD peak: {max_nafld} ({nafld_multiplier})")
    else:
        print("• Data for August 2025 spike is missing or incomplete.")

print(f"\nKey Correlations:")
# Print correlations only if the matrix contains the keywords
if 'MASLD' in correlation_matrix.index and 'NAFLD' in correlation_matrix.columns:
    print(f"• MASLD vs NAFLD: {correlation_matrix.loc['MASLD', 'NAFLD']:.3f}")
if 'MASLD' in correlation_matrix.index and 'Wegovy' in correlation_matrix.columns:
    print(f"• MASLD vs Wegovy: {correlation_matrix.loc['MASLD', 'Wegovy']:.3f}")
if 'MASLD' in correlation_matrix.index and 'Ozempic' in correlation_matrix.columns:
...
for event, results in event_results.items():
    ...
    for drug, stats in results.items():
        ...
        # else: print(f"  • {drug}: Minimal change (<10%)") # Optional: print all changes

print(f"\nAnalysis complete! Files saved in {OUTPUT_DIR}/")
print(f"   • {PLOT_MAIN_PATH.name}")
print(f"   • {PLOT_CORR_PATH.name}")
print(f"   • {CSV_SUMMARY_PATH.name}")

# Show the plots
# plt.show() # Commenting out plt.show() for non-interactive execution

# Save summary statistics to CSV
summary_stats.to_csv(CSV_SUMMARY_PATH)